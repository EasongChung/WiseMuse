/// 行间「自动换行」判定规则。
///
/// 句子跨行合并最初是**无条件**的：只要不跨段落间隙就把下一行接到当前句尾。
/// 实测出现大量误合并——段落末尾的短行会粘上下一段首行、行末带逗号的人工
/// 换行被当成自动换行、缩进的新段首被吞进上一句。
///
/// 本文件把「什么样的换行才是排版自动折行」抽成单一判据 [canMergeLines]，
/// 供三处消费者共用（G2.5.1「只改一侧」教训的制度性防范）：
///
/// - `text_position_service.buildSentences`：PDF 几何侧，度量单位是 PDF 点
/// - `ocr_geometry_service.buildSentences`：OCR 几何侧，度量单位是归一化/像素
/// - `sentence_splitter`：文本侧，无几何信息，**以字符数当坐标**
///   （`charW = 1`，块宽 = 段内最长行字符数，缩进 = 行首空白数）
///
/// 两侧必须走同一函数：朗读单元与高亮单元一旦异源，就会重演错位缺陷。
library;
