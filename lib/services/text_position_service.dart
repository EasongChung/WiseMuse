...
/// 一个句子的几何与文本。
///
/// 分句规则与 `sentence_splitter` 对齐：以 `。！？!?；;` 为硬边界。
/// **句子可以跨行**，但仅当换行处经 `canMergeLines` 判定为排版自动折行时才合并；
/// 合并成立时 [rects] 含多个矩形（每行一个）。
class SentenceBox {
...
/// 句子终止标点（与 `sentence_splitter` 的正则 `(?<=[。！？!?；;])` 对齐）。
...
///
/// 与 `sentence_splitter` 对齐：换行**不是**无条件边界，但也**不是**无条件合并
/// ——只有经 [canMergeLines] 判定为「排版自动折行」的换行处才把下一行接到当前
/// 句尾。合并成立时 [SentenceBox.rects] 每行一个矩形。
///
/// **段落间隙仍是硬边界**——行间垂直间隙 > [paraGapEm]×行高（出现空行）时强制
/// 断句，否则无标点文本（标题、列表等）会把整页合成一句。
///
/// 版心左右边界（[canMergeLines] 的 `blockLeft`/`blockRight`）按**段落块内**统计，
/// 不用整页：多栏排版与页眉页脚会把边界撑到不可用。
