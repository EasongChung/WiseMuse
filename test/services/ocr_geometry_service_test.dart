...
void main() {
  ...
  group('OcrGeometryService', () {
    test('跨行句子正确合并并输出归一化矩形', () {
      ...
      final blocks = [
        block([(0, 0, 60, 10, '春眠不觉晓'), (40, 30, 60, 10, '处处闻啼鸟')]),
      ];
    ...
    test('行末贴边的行合并为一句', () {
      ...
      final blocks = [
        block([(0, 0, 100, 10, '这是第一行文字'), (0, 30, 100, 10, '这是第二行没有标点')]),
      ];
      final out = OcrGeometryService.buildSentences(
        blocks,
        imageWidth: 100,
    ...
    test('句末标点截断为独立句', () {
      final blocks = [
        block([(0, 0, 30, 10, '你好世界。'), (0, 30, 30, 10, '下一个句子')]),
      ];
      final out = OcrGeometryService.buildSentences(
        blocks,
        imageWidth: 100,
    ...
    test('hitSentence 命中归一化坐标并吸附', () {
      ...
      // 点在该行内(归一化 0.3, 0.05)
      final hitIn = OcrGeometryService.hitSentence(
        out,
